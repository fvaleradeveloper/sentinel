...
export default async function WebappLanding({ params }: { params: { webappSlug: string } }) {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {hasActiveSub ? (
        <Link
          href={`/dashboard/${params.webappSlug}`}
          className="inline-block bg-green-600 text-white px-6 py-3 rounded-md hover:bg-green-700"
        >
          Ir a mi cuenta
        </Link>
      ) : (
        <RequestSubscriptionButton webappId={webapp.id} webappSlug={params.webappSlug} />
      )}
      ...
      <div className="mt-12 text-right text-xs text-gray-400">
        <Link href={`/admin/${params.webappSlug}`}>Acceso administrador</Link>
      </div>
    </div>
  )
}